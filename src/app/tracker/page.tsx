...
export default function TrackerPage() {
  ...
  const handleManualExtract = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (inputMode === "url") {
        setExtractError(
          "Link blocked! Try pasting the job description in Text mode.",
        );
        setInputMode("text");
      } else {
        setExtractError(
          "Couldn't read this text. Try copying the core job details again.",
        );
      }
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center text-gray-500">
        ...
        <span className="ml-3 font-serif text-xl italic">
      ...
      <div className="mb-10 z-10 flex flex-col xl:flex-row xl:items-end justify-between gap-8">
        <div>
          ...
          <p className="text-gray-500 dark:text-slate-400 text-lg max-w-md">
            Your personal job tracking universe. Drop a link or paste text to
            begin.
          </p>
        </div>
        ...
        <div className="w-full xl:max-w-xl flex flex-col items-start xl:items-end">
          ...
          <form
            ...
            className="w-full relative flex flex-col gap-2 shadow-sm rounded-[1.5rem]"
          >
            ...
            <button
              type="submit"
              disabled={
                isExtracting || (inputMode === "url" ? !newUrl : !rawText)
              }
              ...
              {isExtracting ? (
                ...
                <>
                  <Sparkle
                    ...
                    weight="fill"
                    className={
                      inputMode === "url" ? "text-rose-400" : "text-rose-500"
                    }
                  />{" "}
                  Extract
                </>
          ...
          {extractError && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="flex items-center gap-2 text-rose-500 mt-3 text-sm font-medium self-start px-2 bg-rose-50 dark:bg-rose-900/20 py-1.5 px-3 rounded-lg border border-rose-100 dark:border-rose-900/50"
            >
      ...
      <div className="flex md:hidden overflow-x-auto gap-2 mb-6 pb-2 hide-scrollbar z-10 sticky top-[70px] bg-slate-50/90 dark:bg-slate-950/90 backdrop-blur-md pt-2">
        {COLUMNS.map((col) => (
          ...
          >
            <col.icon
              size={16}
              weight={activeMobileTab === col.id ? "fill" : "bold"}
            />
            {col.label}
            <span
              className={`ml-1 px-2 py-0.5 rounded-full text-xs ${activeMobileTab === col.id ? "bg-white/50 dark:bg-black/20" : "bg-gray-100 dark:bg-slate-800"}`}
            >
      ...
      <div className="flex-grow flex flex-col md:flex-row gap-6 md:overflow-x-auto pb-8 z-10 hide-scrollbar items-start">
        {COLUMNS.map((column) => {
          ...
          const isMobileHidden =
            activeMobileTab !== column.id ? "hidden md:flex" : "flex";
            ...
            >
              ...
              <div className="flex items-center justify-between mb-6 px-2">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded-xl ${column.activeBg} ${column.color}`}
                  >
                ...
                <AnimatePresence>
                  {columnJobs.map((job) => (
                    ...
                    >
                      ...
                      {/* Top area: Title and Actions (Link + Delete) */}
                      <div className="flex justify-between items-start mb-1 gap-2">
                        <h3 className="font-bold text-gray-900 dark:text-slate-50 leading-tight text-lg flex-grow">
                          {job.title}
                        </h3>

                        {/* 🚨 THE SECSY ACTION DOCK (Fades & Slides in on Hover) */}
                        <div className="flex items-center gap-1.5 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all duration-300 transform md:translate-x-2 md:group-hover:translate-x-0 flex-shrink-0">
                          {/* The Trash Button */}
                          <button
                            onClick={() => handleDeleteJob(job.id)}
                            className="text-gray-400 hover:text-red-500 bg-gray-50 hover:bg-red-50 dark:bg-slate-800 dark:hover:bg-red-900/40 p-2 rounded-full transition-all hover:scale-110 active:scale-95"
                          ...
                          {job.url && (
                            <a
                              ...
                              className="text-gray-400 hover:text-rose-500 bg-gray-50 hover:bg-rose-50 dark:bg-slate-800 dark:hover:bg-rose-900/30 p-2 rounded-full transition-all hover:scale-110 active:scale-95"
                              title="Open Job Link"
                            >
                      ...
                      {job.tech_stack && job.tech_stack.length > 0 && (
                        <div className="flex flex-wrap gap-2 mb-4">
                          {job.tech_stack.slice(0, 4).map((tech, i) => (
                            <span
                              key={i}
                              className="px-2.5 py-1 bg-gray-100/80 dark:bg-slate-800/80 text-gray-600 dark:text-slate-300 text-[11px] rounded-lg border border-gray-200/50 dark:border-slate-700/50 font-semibold"
                            >
                              {tech}
                            </span>
                          ))}
                            ...
                            >
                              {job.status === c.id ? (
                                ...
                                <c.icon
                                  size={16}
                                  weight="fill"
                                  className="group-hover/dockbtn:scale-110 transition-transform"
                                />
                              )}
                            </button>
                          ))}
